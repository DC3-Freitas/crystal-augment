...
def ranking_loss(
    ...
    pairs: list[
        Tuple[int, int]
    ],  # list of (idx_a, idx_b) where a should be greater than b
    margin: float = 0.1,
):
    if len(pairs) == 0:
    ...
    for idx_a, idx_b in pairs:
        loss += torch.relu(s[idx_b] - s[idx_a] + margin)
    ...
    for i in range(batch.num_graphs):
        for j in range(i + 1, batch.num_graphs):
            ...
            if same_family and same_chem and same_struct:
                # This is a little confusing, since
                # s_hat predicts degree of order, and sigma is degree of perturbation.
                # So it may seem that we are putting the pairs in backwards.
...
def batch_invariance_loss(
    ...
    for idx_a, idx_b in pair_indices:
        total = total + nn.MSELoss()(embeddings[idx_a], embeddings[idx_b])
