...
def batch_invariance_loss(embeddings, batch, pair_indices):
    ...
    for idx_a, idx_b in pair_indices:
        ...
        total += invariance_loss(z_a, z_b)
    return total / max(len(pair_indices), 1)

def ranking_loss(
    s_a: torch.Tensor, # (N,) less-perturbed
    s_b: torch.Tensor, # (N,) more-perturbed
    margin: float = 0.1,
) -> torch.Tensor:
    # TODO: s is the amount of order?
    return torch.relu(-(s_a - s_b) + margin).mean()
...
def bravais_classification_loss(y_bravais: torch.Tensor, batch: torch_geometric.data.Batch):
...
class TotalLoss(nn.Module):
    def __init__(
        ...
        self.weights = {"var_S": lambda_var, "var_C": lambda_var}
    ...
    def forward(
        ...
        loss_dict = {}
        loss_dict["cls_S"] = bravais_classification_loss(outputs["y_bravais"], batch)
        loss_dict["var_S"] = vicreg_variance_loss(outputs["z_S"])
        loss_dict["var_C"] = vicreg_variance_loss(outputs["z_C"])
        # ... etc ...
